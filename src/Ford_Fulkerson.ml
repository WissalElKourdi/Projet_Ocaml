...
(* fonction qui cherche le min dans un chemin donné*)
...
let rec print_id li_id = match li_id with
  ...
  (* add_flow est une fonction qui rajoute une valeur v à tous les arcs d'un chemin donné *)
...
(*flow_max est une fonction qui à chaque fois qu'elle trouve un chemin, elle calcule le min et le rajoute à ses arcs, jusqu'a ce qu'eklle ne trouve plus de chemin *)
